...
//            val bobNode = net.createPartyNode(notaryNode.info.address, BOB.name)
    ...
    private fun runBuyerAndSeller(notaryNode: MockNetwork.MockNode,
        ...
        val seller = SellerInitiator(buyerNode.info.legalIdentity, notaryNode.info, assetToSell)
        val sellerResult = sellerNode.services.startFlow(seller).resultFuture
        return RunResult(firstBuyerFiber, sellerResult, seller.stateMachine.id)
    }

    @InitiatingFlow
    class SellerInitiator(val buyer: Party, val notary: NodeInfo, val assetToSell: StateAndRef<OwnableState>) : FlowLogic<SignedTransaction>() {
        @Suspendable
        override fun call(): SignedTransaction {
            send(buyer, notary.notaryIdentity)
    ...
    class BuyerAcceptor(val seller: Party) : FlowLogic<SignedTransaction>() {
        @Suspendable
        override fun call(): SignedTransaction {
            val notary = receive<Party>(seller).unwrap {
                require(serviceHub.networkMapCache.isNotary(it)) { "$it is not a notary" }
                it
            }
            return subFlow(Buyer(seller, notary, 1000.DOLLARS, CommercialPaper.State::class.java))
        }
    }
