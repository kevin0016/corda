...
object CashIssueFlow {
    ...
    class Initiator(val amount: Amount<Currency>,
        ...
        override fun call(): SignedTransaction {
            ...
            val theirCert = theirCertPath.certificates.last()
            progressTracker.currentStep = GENERATING_TX
            val builder: TransactionBuilder = TransactionType.General.Builder(notary = null)
            val issuer = serviceHub.myInfo.legalIdentity.ref(issueRef)
            // TODO: Get a transaction key, don't just re-use the owning key
            Cash().generateIssue(builder, amount.issuedBy(issuer), AnonymousParty(theirCert.publicKey), notary)
    ...
    class Recipient(val issuer: Party): FlowLogic<Unit>() {
        override fun call() {
            // TODO: We should verify we're expecting this cash, not just blindly create a new key and certificate to
            //       accept it. Creating and storing keys is expensive, and arbitrarily accepting cash is a KYC risk.
            TxKeyFlowUtilities.provideKey(this, issuer, false)
        }
    }
}
