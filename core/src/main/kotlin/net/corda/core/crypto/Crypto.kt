...
object Crypto {
    ...
    private val algorithmMultiMap = buildAlgorithmMultiMap()

    // Build a multimap to map algorithm names with signature schemes.
    // We need a multimap as EC (or ECDSA) keys are used for both ECDSA_SECP256K1_SHA256 and ECDSA_SECP256R1_SHA256.
    private fun buildAlgorithmMultiMap(): ImmutableMultimap<String, SignatureScheme> {
        val builder = ImmutableMultimap.builder<String, SignatureScheme>()
        for (signatureScheme in Crypto.supportedSignatureSchemes.values) {
            builder.put(signatureScheme.algorithmName, signatureScheme)
        }
    ...
    fun findSignatureScheme(key: Key): SignatureScheme {
        ...
        algorithmMultiMap.get(algorithm)
                .filter { validateKey(it, key) }
                .firstOrNull { return it ?: throw IllegalArgumentException("Key algorithm: ${key.algorithm} is supported, but the key cannot be validated. Please check the key params!") }
    ...
    fun decodePrivateKey(encodedKey: ByteArray): PrivateKey {
        ...
        for (signatureScheme in algorithmMultiMap[algorithm]) {
            try {
    ...
    private fun validatePublicKey(signatureScheme: SignatureScheme, key: PublicKey): Boolean {
        if (key is EdDSAPublicKey || key is BCECPublicKey) {
            return publicKeyOnCurve(signatureScheme, key)
        } else if (key is BCRSAPublicKey || key is BCSphincs256PublicKey) {
            return true // TODO: Check if non-ECC keys satisfy params (i.e. approved/valid RSA modulus size).
        } else throw IllegalArgumentException("Unsupported key type: ${key::class}")
    }
    ...
    private fun validatePrivateKey(signatureScheme: SignatureScheme, key: PrivateKey): Boolean {
        if (key is BCECPrivateKey) {
            return key.parameters == signatureScheme.algSpec
        } else if (key is EdDSAPrivateKey) {
            return key.params == signatureScheme.algSpec
        } else if (key is BCRSAPrivateKey || key is BCSphincs256PrivateKey) {
            return true // TODO: Check if non-ECC keys satisfy params (i.e. approved/valid RSA modulus size).
        } else throw IllegalArgumentException("Unsupported key type: ${key::class}")
    }
